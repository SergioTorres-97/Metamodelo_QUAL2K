...
MODELO_PATH   = OUTPUT_DIR / "xgboost_dbo.joblib"
FIGS_DIR      = OUTPUT_DIR / "figuras_xgb"
...
def entrenar(
    ...
    subsample:        float = 0.8,
    colsample:        float = 0.8,
    reg_alpha:        float = 0.1,     # L1 — induce sparsity
    reg_lambda:       float = 1.0,     # L2 — penaliza pesos grandes
    min_child_weight: int   = 5,       # mínimo de muestras por hoja
    gamma:            float = 0.1,     # ganancia mínima para partir nodo
    early_stopping:   int   = 40,      # rondas sin mejora en val → parar
    test_size:        float = 0.20,
    val_size:         float = 0.10,
    seed:             int   = 42,
    n_perm:           int   = 30,      # repeticiones permutation importance
    buscar:           bool  = False,
    n_trials:         int   = 50,
) -> XGBRegressor:
    ...
    # Features — XGBoost es invariante a la escala; se usan los valores directos
    ...
    print(f"[XGB] Arquitectura  : {len(FEATURES)} features → {n_estimators} árboles máx.")
    print(f"[XGB] Regularización: alpha={reg_alpha}  lambda={reg_lambda}  "
          f"gamma={gamma}  min_child_weight={min_child_weight}")
    print(f"[XGB] Early stopping: {early_stopping} rondas sin mejora en val\n")
    ...
    arboles_usados = modelo.best_iteration + 1
    print(f"\n[XGB] Mejor iteración : {arboles_usados} árboles  "
          f"(val RMSE = {modelo.best_score:.4f})")
    ...
    print(f"\n[XGB] Calculando permutation importance ({n_perm} repeticiones)...")
    ...
    _exportar_excel(
        ...
        config         = dict(
            ...
            gamma=gamma, arboles_usados=arboles_usados,
    ...
    print(f"[XGB] Modelo guardado  : {MODELO_PATH}")
...
def _buscar_optuna(
    ...
    """
    Optimiza hiperparametros de XGBoost con TPE sobre el mismo split
    ...
    print(f"\n[XGB] Busqueda bayesiana Optuna: {n_trials} trials "
          f"(split unico train/val, early stopping={early_stopping})...")
    t0 = time.time()
    study.optimize(objective, n_trials=n_trials, show_progress_bar=False)

    print(f"[XGB] Busqueda completada en {time.time() - t0:.1f} s")
    print(f"[XGB] Mejor R2 val (log1p): {study.best_value:.4f}")
    print("[XGB] Mejores hiperparametros:")
...
def _grafica_curvas(evals: dict):
    """Curva de aprendizaje RMSE train vs val por ronda de boosting."""
    # La clave coincide con el eval_metric declarado en el constructor ("rmse")
    metric_key = next(iter(evals["validation_0"]))
    rmse_train = evals["validation_0"][metric_key]
    rmse_val   = evals["validation_1"][metric_key]
    ...
    plt.close()
    print(f"[XGB] Figura guardada: {path}")
...
def _grafica_obs_vs_pred(y_true, y_pred):
    ...
    ax.set_ylabel("DBO predicha XGBoost (mg/L)", fontweight="bold", fontsize=11)
...
def _grafica_importancia_nativa(modelo: XGBRegressor):
    """Importancia nativa XGBoost basada en ganancia de los splits."""
    ...
    ax.set_title("Importancia nativa — XGBoost (ganancia)",
                 fontweight="bold", fontsize=12)
    ax.minorticks_on()
...
def _grafica_importancia_permutation(imp_mean: pd.Series, imp_std: pd.Series):
    """Permutation Importance — caída en R² al permutar cada feature."""
    ...
    ax.set_title("Importancia de variables — XGBoost (Permutation Importance)\n"
...
def _exportar_excel(
    ...
    excel_path = OUTPUT_DIR / "resultados_xgb.xlsx"
    ...
    print(f"[XGB] Excel guardado   : {excel_path}")
